...
use up_rust::{
    PayloadEncoding, PreparedTxLoanSpec, StaticUriProvider, UCode, UEncodedZeroCopyListener,
    UFrameMetadata, UFrameView, UMessageBuilder, UStatus, UTxBuffer, UTxLoanSpec, UUninitTxBuffer,
    UUri, UVecRxLease, UVecTxBuffer, UVecUninitTxBuffer, UZeroCopyListener, UZeroCopyTransportCore,
...
async fn physical_islands_require_bridge_and_do_not_reflect(
    ...
    let root = std::env::var("UP_ICEORYX2_TEST_ROOT").unwrap_or_else(|_| {
...
struct InstrumentedUninitTxBuffer {
    inner: UVecUninitTxBuffer,
    encoded_metadata: Option<Vec<u8>>,
    instrumentation: Arc<RouteInstrumentation>,
}

impl UUninitTxBuffer for InstrumentedUninitTxBuffer {
    type Initialized = InstrumentedTxBuffer;

    fn metadata(&self) -> &UFrameMetadata {
        self.inner.metadata()
    }

    fn payload_uninit_mut(&mut self) -> &mut [std::mem::MaybeUninit<u8>] {
        self.inner.payload_uninit_mut()
    }

    unsafe fn assume_payload_initialized(self) -> Self::Initialized {
        InstrumentedTxBuffer {
            // SAFETY: the caller guarantees every payload byte is initialized.
            inner: unsafe { self.inner.assume_payload_initialized() },
...
impl UZeroCopyTransportImpl for RecordingZeroCopyTransport {
    ...
    async fn send_validated_zero_copy(&self, buffer: Self::Tx) -> Result<(), UStatus> {
        ...
        self.sent
            .lock()
            .expect("recording transport lock poisoned")
            .push(UVecRxLease::new(buffer.metadata().clone(), payload)?);
        Ok(())
    }
    ...
    async fn receive_validated_zero_copy(
        ...
        Err(UStatus::fail_with_code(UCode::NotFound, "no queued frame"))
    }
...
impl UZeroCopyTransportImpl for InstrumentedZeroCopyTransport {
    ...
    async fn loan_validated_tx(&self, spec: UTxLoanSpec) -> Result<Self::Tx, UStatus> {
        ...
        Ok(InstrumentedTxBuffer {
            inner: UVecTxBuffer::with_alignment(
                spec.metadata().clone(),
                spec.payload_len(),
                spec.payload_alignment_proof().as_usize(),
            )?,
...
impl UZeroCopyTransportCore for SelectedWireCore {
    ...
    async fn send_prepared_zero_copy(&self, buffer: Self::Tx) -> Result<(), UStatus> {
        ...
        let frame = self.dispatch_sent.then(|| EncodedRxLease {
            ...
            slices: vec![buffer.payload().to_vec()],
        });
        let listeners = {
            let mut state = self
                .state
                .lock()
                .expect("selected-wire core state lock poisoned");
            state.sent_metadata.push(buffer.metadata().clone());
            state.sent_payloads.push(buffer.payload().to_vec());
            if frame.is_some() {
...
#[async_trait]
impl UZeroCopyUninitTransportCore for SelectedWireCore {
    type UninitTx = InstrumentedUninitTxBuffer;
...
#[repr(C)]
#[derive(
    Clone, Copy, Debug, Default, Eq, PartialEq, up_rust::StablePayload, up_rust::StablePayloadInit,
)]
#[stable_payload(type_name = "up_streamer.tests.StableBytes")]
struct StableBytes {
    bytes: [u8; 4],
}
...
fn instrumented_payload_frame(
    ...
    .build_with_payload(Bytes::from(vec![0; payload_len]), PayloadEncoding::PROTOBUF)
    .expect("message");
    let metadata = try_project_umessage_to_frame_metadata(&message).expect("metadata");
    InstrumentedRxLease {
        metadata,
...
fn present_empty_payload_frame() -> UVecRxLease {
    ...
    .build_with_payload(Bytes::new(), PayloadEncoding::PROTOBUF)
    .expect("message");
    let metadata = try_project_umessage_to_frame_metadata(&message).expect("metadata");
    UVecRxLease::new(metadata, Some(Vec::new())).expect("zero-copy frame")
}

async fn wait_for_sent_count(transport: &RecordingZeroCopyTransport, expected: usize) {
    for _ in 0..20 {
...
async fn zero_copy_l2_stable_publish_routes_through_streamer() {
    ...
    publisher
        .publish_stable::<StableBytes, _>(
            0x8001,
            CallOptions::for_publish(None, None, None),
            |payload| {
                payload
                    .into_initializer()
                    .bytes_from_slice(b"zcpy")
                    .expect("stable bytes")
                    .finish()
            },
        )
