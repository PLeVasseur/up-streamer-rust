...
use up_rust::{
    ...
    UMessageBuilder, UOwnedFrame, UOwnedListener, UOwnedTransportImpl, UStatus, UUri,
};
...
fn custom_payload_frame() -> UOwnedFrame {
    ...
    let metadata = try_project_umessage_to_frame_metadata(&message)
        .expect("metadata")
        .with_payload_encoding(PayloadEncoding::from_registry_entry(9))
...
async fn owned_l2_publish_routes_through_streamer() {
    ...
    publisher
        .publish(
            ...
            Some(UPayload::new("owned publish", PayloadEncoding::TEXT)),
        )
        .await
...
async fn owned_l2_notifications_route_through_streamer() {
    ...
    ingress_l2
        ...
        .notify(
            ...
            Some(UPayload::new("owned notification", PayloadEncoding::TEXT)),
        )
        .await
...
async fn owned_l2_rpc_routes_through_streamer() {
    ...
    let response = timeout(
        ...
        client_l2.rpc_client().invoke_method(
            ...
            Some(UPayload::new("owned rpc", PayloadEncoding::TEXT)),
        ),
    )
    .await
