...
struct Cli {
    ...
    #[arg(long, value_enum, default_value = "notify-receiver", hide = true)]
    role: FlowRole,
    #[arg(long)]
    ...
    #[arg(long, default_value = "zenoh-notifyee")]
    payload: String,
    #[arg(long, default_value_t = 1)]
...
async fn main() -> Result<(), UStatus> {
    ...
    match cli.mode {
        ...
        None => run_classic_zenoh_notifyee(&cli).await,
    }
}

async fn run_classic_zenoh_notifyee(cli: &Cli) -> Result<(), UStatus> {
    ...
    let source_filter = topic_uri(cli.peer_authority.as_str(), cli.topic_resource_id)?;
    let sink_filter = endpoint_uri(cli.local_authority.as_str())?;
    let listener = common::native::payload_listener(&cli.native, Arc::new(PublishReceiver));
    transport
...
async fn run_zero_copy(cli: &Cli) -> Result<(), UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "copy_minimized zenoh-notifyee role for {:?}/{:?} is not available with this feature set",
...
async fn owned_transport(cli: &Cli) -> Result<Arc<dyn UOwnedTransport>, UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "owned_frame zenoh-notifyee role for {:?}/{:?} is not available with this feature set",
            cli.transport, cli.wire_format
        ))),
    }
}
...
fn required_lola_mw_com_config_path(cli: &Cli) -> Result<String, UStatus> {
    let path = cli.lola_mw_com_config_file.as_deref().ok_or_else(|| {
        invalid_config("--lola-mw-com-config-file is required for LoLa zenoh-notifyee roles")
    })?;
...
async fn receive_passive_owned_frame(
    ...
    if cli.transport != FlowTransport::Zenoh {
        println!("READY listener_registered");
        return receive_owned_frame(transport, source_filter, sink_filter, cli).await;
    }
    ...
    let frame = receive_listener_frame(&mut rx, cli.timeout_ms, "owned frame").await?;
    if cli.wire_format == FlowWireFormat::Native {
        cli.native
    ...
    T::Rx: UZeroCopyRxLease,
{
    ...
    transport
        ...
        .await?;
    print_zenoh_listener_ready();
    let frame = receive_listener_frame(&mut rx, cli.timeout_ms, "zero-copy frame").await?;
