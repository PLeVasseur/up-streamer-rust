...
use up_rust::{
    try_project_umessage_to_frame_metadata, PayloadEncoding, PayloadFormat, StableContainerPayload,
    UCode, UFrameMetadata, UFrameView, UListener, UMessage, UMessageBuilder, UOwnedFrame,
    UOwnedListener, UOwnedTransport, UStatus, UTransport, UTxBuffer, UTxLoanSpec, UUri,
...
const NATIVE_PAYLOAD_MAGIC: u32 = u32::from_le_bytes(*b"DDSR");
...
struct Args {
    ...
    #[arg(long, default_value = "authority-b")]
...
fn payload(args: &Args) -> Result<Vec<u8>, UStatus> {
    match args.encoding {
        ...
        Encoding::Xcdrv2 => xcdrv2_payload_bytes(1, args.local_authority.clone(), &args.payload),
