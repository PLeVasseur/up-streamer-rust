...
use up_rust::{
    ...
    UFrameView, UMessage, UMessageBuilder, UOwnedFrame, UOwnedTransport, UStatus, UTxBuffer,
...
struct Cli {
    ...
    mode: FlowMode,
    #[arg(long, value_enum, default_value = "lola", hide = true)]
    transport: FlowTransport,
    #[arg(long = "encoding", value_enum, default_value = "protobuf")]
    wire_format: FlowWireFormat,
    #[arg(long, value_enum, default_value = "rpc-client", hide = true)]
    role: FlowRole,
    #[arg(long)]
    ...
    #[arg(long, default_value = "lola-client")]
    payload: String,
    #[arg(long, default_value_t = 1)]
...
async fn run_zero_copy(cli: &Cli) -> Result<(), UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "copy_minimized lola-client role for {:?}/{:?} is not available with this feature set",
...
async fn owned_transport(cli: &Cli) -> Result<Arc<dyn UOwnedTransport>, UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "owned_frame lola-client role for {:?}/{:?} is not available with this feature set",
            cli.transport, cli.wire_format
        ))),
    }
}
...
fn required_lola_mw_com_config_path(cli: &Cli) -> Result<String, UStatus> {
    let path = cli.lola_mw_com_config_file.as_deref().ok_or_else(|| {
        invalid_config("--lola-mw-com-config-file is required for LoLa lola-client roles")
    })?;
