...
struct Cli {
    ...
    #[arg(long, value_enum, default_value = "rpc-server", hide = true)]
    role: FlowRole,
    #[arg(long)]
    ...
    #[arg(long, default_value = "zenoh-server")]
    payload: String,
    #[arg(long, default_value_t = 1)]
...
async fn run_classic_zenoh_server(cli: &Cli) -> Result<(), UStatus> {
    ...
    service
        .register_listener(
            &source_filter,
            Some(&sink_filter),
            common::native::payload_listener(&cli.native, service_request_responder.clone()),
        )
        .await?;
...
async fn run_zero_copy(cli: &Cli) -> Result<(), UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        #[cfg(feature = "iceoryx2-zero-copy")]
        (FlowTransport::Iceoryx2, FlowWireFormat::Arrow) => {
            run_zero_copy_transport(
        ...
        #[cfg(feature = "iceoryx2-zero-copy")]
        (FlowTransport::Iceoryx2, FlowWireFormat::Omgidl) => {
            run_zero_copy_transport(
        ...
        _ => Err(invalid_config(format!(
            "copy_minimized zenoh-server role for {:?}/{:?} is not available with this feature set",
...
async fn owned_transport(cli: &Cli) -> Result<Arc<dyn UOwnedTransport>, UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "owned_frame zenoh-server role for {:?}/{:?} is not available with this feature set",
            cli.transport, cli.wire_format
        ))),
    }
}
...
fn required_lola_mw_com_config_path(cli: &Cli) -> Result<String, UStatus> {
    let path = cli.lola_mw_com_config_file.as_deref().ok_or_else(|| {
        invalid_config("--lola-mw-com-config-file is required for LoLa zenoh-server roles")
    })?;
