...
use std::thread;
use up_rust::{
    ComparableListener, UAttributes, UCode, UListener, UMessage, UMessageType, UStatus,
    UTransport, UUri,
};
...
pub struct UPClientFoo {
    ...
    listeners: Arc<Mutex<HashMap<(UUri, Option<UUri>), HashSet<ComparableListener>>>>,
...
impl UPClientFoo {
    pub async fn new(
        ...
        let listeners: Arc<Mutex<HashMap<(UUri, Option<UUri>), HashSet<ComparableListener>>>> =
            Arc::new(Mutex::new(HashMap::new()));
        let authority_listeners: Arc<Mutex<HashMap<String, HashSet<ComparableListener>>>> =
            Arc::new(Mutex::new(HashMap::new()));
            ...
            Some(source) => {
                ...
                if let Some(topic_listeners) = topic_listeners {
                    ...
                    debug!(
                        ...
                        name, attr.source.as_ref(), attr.sink.as_ref()
                    );
                }
            }
        }
...
impl UTransport for UPClientFoo {
    ...
    async fn register_listener(
        ...
        return if source_filter.authority_name != "*" && sink_for_any {
            ...
            debug!("{}: registering regular listener for: source: {:?} sink: {:?}", &self.name, source_filter, sink_filter);

            let mut listeners = self.listeners.lock().await;
            let topic_listeners = listeners.entry((source_filter.clone(), sink_filter.cloned())).or_default();
            let comparable_listener = ComparableListener::new(listener);
            let inserted = topic_listeners.insert(comparable_listener);

            match inserted {
                true => {
                    debug!("{}: successfully registered regular listener for: source: {:?} sink: {:?}", &self.name, source_filter, sink_filter);
    ...
    async fn unregister_listener(
        ...
        debug!("{} unregistering listener for source_filter: {source_filter:?}", &self.name);
        ...
        return if source_filter.authority_name != "*" && sink_for_any {
            ...
            let mut listeners = self.listeners.lock().await;
            let Some(topic_listeners) = listeners.get_mut(&(source_filter.clone(), sink_filter.cloned())) else {
