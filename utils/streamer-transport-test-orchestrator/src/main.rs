...
fn push_endpoint(
    ...
    if profile.physical == PhysicalTransport::Vsomeip {
        value["payload_encoding_id"] = json!(encoding.payload_encoding_id());
    }
...
fn vsomeip_app_ids(row: &MatrixRow, base: u16) -> VsomeipAppIds {
    ...
    // SOME/IP requests and notifications reconstruct their source entity from
    // the sending application's client ID at the receiving physical hop.
    if matches!(
        row.role,
        RoleStyle::ClientServerRpc | RoleStyle::NotifierNotifyee
    ) {
...
mod tests {
    ...
    #[test]
    fn vsomeip_generated_client_ids_preserve_role_source_entity() {
        let base = std::env::temp_dir().join(format!(
            "streamer-vsomeip-identity-{}-{}",
            std::process::id(),
            Utc::now().timestamp_nanos_opt().unwrap()
        ));
        let cli = Cli::try_parse_from(["orchestrator"]).unwrap();
        let mut checked = 0;
        for row in matrix_rows().into_iter().filter(|row| {
            matches!(
                row.role,
                RoleStyle::ClientServerRpc | RoleStyle::NotifierNotifyee
            ) && ((row.source.physical == PhysicalTransport::Vsomeip
                && row.sink.physical == PhysicalTransport::Mqtt5)
                || (row.source.physical == PhysicalTransport::Mqtt5
                    && row.sink.physical == PhysicalTransport::Vsomeip))
                && support_status(row).classification == RowClassification::Pass
        }) {
            let directory = base.join(&row.id);
            fs::create_dir_all(&directory).unwrap();
            let paths = VsomeipConfigPaths::new(&directory);
            write_vsomeip_configs(&row, &paths).unwrap();
            let source_command = role_command(
                &row,
                true,
                &ZenohConfigPaths::new(&directory),
                &paths,
                None,
                "unused",
                &cli,
            ...
            .unwrap();
            let source_entity = &source_command
                .args
                .windows(2)
                .find(|pair| pair[0] == "--uentity")
                .unwrap()[1];
            let physical_sender = if row.source.physical == PhysicalTransport::Vsomeip {
                &paths.source
            } else {
                &paths.streamer
            };
            let config: serde_json::Value =
                serde_json::from_slice(&fs::read(physical_sender).unwrap()).unwrap();
            let applications = config["applications"].as_array().unwrap();
            assert_eq!(
                applications[0]["id"].as_str().unwrap().to_ascii_lowercase(),
                source_entity.to_ascii_lowercase(),
                "SOME/IP client identity must preserve the source entity: {}",
                row.id
            );
            let ids: BTreeSet<_> = applications
                .iter()
                .map(|application| application["id"].as_str().unwrap())
                .collect();
            assert_eq!(ids.len(), applications.len(), "distinct process IDs");
            checked += 1;
        }
