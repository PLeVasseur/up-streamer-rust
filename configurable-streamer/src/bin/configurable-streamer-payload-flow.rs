...
use up_rust::{
    PayloadCodecIdentity, PayloadEncoding, StablePayloadInit, UCode, UFrameMetadata, UFrameView,
...
const NATIVE_FLOW_PAYLOAD_CAPACITY: usize = 256;
const NATIVE_FLOW_PAYLOAD_MAGIC: u32 = u32::from_le_bytes(*b"UPNF");

use configurable_streamer_wire_support::native_profile::{NativeFlowPayload, NativePayloadContext};

type NativeContext = NativePayloadContext<NativeFlowPayload>;
...
struct Cli {
    ...
    #[arg(long, default_value = "payload-flow")]
    payload: String,
    #[arg(long, default_value_t = 1)]
...
async fn run_zero_copy(cli: &Cli) -> Result<(), UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "copy_minimized payload-flow role for {:?}/{:?} is not available with this feature set",
    ...
    T::Rx: UZeroCopyRxLease,
{
    match cli.role {
        ...
        FlowRole::RpcServer => {
            ...
            let payload =
                configurable_streamer_wire_support::payload_bytes::copy_payload_bytes(&request)?;
            let response_metadata = response_metadata(cli, request.metadata())?;
            tokio::time::sleep(Duration::from_millis(cli.rpc_response_delay_ms)).await;
...
async fn owned_transport(cli: &Cli) -> Result<Arc<dyn UOwnedTransport>, UStatus> {
    match (cli.transport, cli.wire_format) {
        ...
        _ => Err(invalid_config(format!(
            "owned_frame payload-flow role for {:?}/{:?} is not available with this feature set",
            cli.transport, cli.wire_format
        ))),
    }
}
...
fn required_lola_mw_com_config_path(cli: &Cli) -> Result<String, UStatus> {
    let path = cli.lola_mw_com_config_file.as_deref().ok_or_else(|| {
        invalid_config("--lola-mw-com-config-file is required for LoLa payload-flow roles")
    })?;
    ...
    T::Rx: UZeroCopyRxLease,
{
    receive_zero_copy_frame(transport, source_filter, sink_filter, timeout_ms)
        .await
        .and_then(|frame| {
            configurable_streamer_wire_support::payload_bytes::copy_payload_bytes(&frame)
        })
}
    ...
    T::Rx: UZeroCopyRxLease,
{
    ...
    loop {
        ...
        match tokio::time::timeout(
            remaining.min(Duration::from_millis(100)),
            transport.receive_validated_zero_copy(source_filter, sink_filter),
        )
        .await
        {
            Ok(Ok(frame)) => {
...
fn payload_bytes(cli: &Cli) -> Result<Vec<u8>, UStatus> {
    match cli.wire_format {
        FlowWireFormat::Native => native_flow_payload_bytes(cli),
        #[cfg(any(
            feature = "zenoh-zero-copy",
            feature = "iceoryx2-zero-copy",
            feature = "lola-transport",
        ...
        FlowWireFormat::Xcdrv2 => Ok(VEHICLE_SIGNAL_V1_GOLDEN_BYTES.to_vec()),
...
fn payload_alignment(cli: &Cli) -> usize {
    match cli.wire_format {
        FlowWireFormat::Native => mem::align_of::<NativeFlowPayload>(),
        _ => cli.payload_alignment,
    }
}

fn native_flow_payload_bytes(cli: &Cli) -> Result<Vec<u8>, UStatus> {
    let payload = cli.payload.as_bytes();
    if payload.len() > NATIVE_FLOW_PAYLOAD_CAPACITY {
        return Err(invalid_config(format!(
            "native stable payload input is {} bytes, maximum is {} bytes",
            payload.len(),
            NATIVE_FLOW_PAYLOAD_CAPACITY
        )));
    }

    let mut payload_bytes = [0_u8; NATIVE_FLOW_PAYLOAD_CAPACITY];
    payload_bytes[..payload.len()].copy_from_slice(payload);

    let mut storage = MaybeUninit::<NativeFlowPayload>::uninit();
    let init =
        NativeFlowPayload::init(uninit_bytes(&mut storage)).map_err(stable_payload_init_error)?;
    let initialized = init
        .magic(NATIVE_FLOW_PAYLOAD_MAGIC)
        .schema_version(1)
        .payload_len(payload.len() as u32)
        .payload_checksum(payload_checksum(payload))
        .payload_bytes_from_array(&payload_bytes)
        .finish();
...
fn stable_payload_init_error(error: up_rust::UWireError) -> UStatus {
    invalid_config(format!(
        "failed to initialize native stable payload: {error}"
    ))
}
