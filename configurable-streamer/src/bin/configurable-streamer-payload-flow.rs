...
use up_rust::selected_wire_user_api::StableContainerWireFormat;
use up_rust::{
    PayloadEncoding, PayloadFormat, StableContainerPayload, StablePayloadInit, UCode,
    UFrameMetadata, UFrameView, UMessage, UMessageBuilder, UOwnedFrame, UOwnedTransport, UStatus,
...
struct NativeFlowPayload {
    magic: u32,
    schema_version: u32,
    payload_len: u32,
    payload_checksum: u32,
    payload_bytes: [u8; NATIVE_FLOW_PAYLOAD_CAPACITY],
}
...
fn payload_encoding(wire_format: FlowWireFormat) -> PayloadEncoding {
    match wire_format {
        FlowWireFormat::Native => StableContainerPayload::<NativeFlowPayload>::encoding(),
...
fn native_flow_payload_bytes(cli: &Cli) -> Result<Vec<u8>, UStatus> {
    ...
    let init = NativeFlowPayload::init_from_uninit_bytes(uninit_bytes(&mut storage))
        .map_err(stable_payload_init_error)?;
