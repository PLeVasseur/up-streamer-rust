...
fn resolve_payload_encoding(
    ...
    endpoint_config
        .payload_encoding_id
        .map(up_rust::PayloadEncoding::from_id)
        .transpose()
        .map_err(|error| {
            invalid_config(format!(
                "endpoint {}: invalid payload_encoding_id: {error}",
                endpoint_config.endpoint
            ))
        })
}
...
fn resolve_vsomeip_payload_encoding(
    ...
    for endpoint_config in endpoint_configs {
        let Some(encoding) = resolve_payload_encoding(endpoint_config)? else {
            return Err(invalid_config(format!(
                "vSomeIP endpoint {} requires payload_encoding_id: the \
            ...
            if existing != &encoding {
                return Err(invalid_config(format!(
                    "all vSomeIP endpoints in one streamer process must use the same \
                     payload_encoding_id; endpoint {} uses `{}` but earlier \
...
fn register_vsomeip_endpoints(
    ...
    for endpoint_config in endpoint_configs {
        ...
        if endpoint_config.payload_encoding_id.is_none() {
            return Err(invalid_config(format!(
                "vSomeIP endpoint {} requires payload_encoding_id: the \
                 SOME/IP wire carries no payload-encoding metadata, so the \
                 per-topic convention must be declared explicitly",
...
async fn main() -> Result<(), UStatus> {
    ...
    let vsomeip_transport: Option<Arc<dyn UTransport>> = match &config.transports.vsomeip {
        ...
        Some(vsomeip) => {
            let payload_encoding = resolve_vsomeip_payload_encoding(&vsomeip.endpoints)?;
            Some(Arc::new(
                up_transport_vsomeip::UPTransportVsomeip::new_with_config_and_transport_config(
                    ...
                    None,
                    up_transport_vsomeip::TransportConfig::new(payload_encoding),
                )
                .map_err(|e| {
                    UStatus::fail_with_code(
                        UCode::Internal,
