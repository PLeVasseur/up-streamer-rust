...
async fn main() {
    ...
    let cloud_ip = vec![192, 168, 3, 200];
    ...
    let utransport_mqtt = UTransportMqtt::new_from_config(config).await.unwrap();
    ...
    let timer_hour_uuri = UUri {
        authority: Some(UAuthority {
            remote: Some(Remote::Ip(cloud_ip.clone())),
        }),
