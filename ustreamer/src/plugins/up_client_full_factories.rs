...
use crate::plugins::up_client_full::{
    UpClientFull, UpClientFullFactory, UpClientFullFactoryFunction,
};
    ...
    fn create_up_client(&self) -> UpClientFullFactoryFunction {
        Box::new(|| {
            Box::pin(async move {
                ...
                let up_client: Box<dyn UpClientFull> =
                    Box::new(ULinkZenoh::new_from_config(up_client_config).await.unwrap());
                up_client
            })
        })
    }
        ...
        Box::new(|| {
            Box::pin(async move {
                ...
                up_client_config
                    .set_mode(Some(WhatAmI::Peer))
                    .expect("Unable to configure as Peer");
                let up_client: Box<dyn UpClientFull> = Box::new(
